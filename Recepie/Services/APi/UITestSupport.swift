...
extension URLSession: AddLocalTestFile {
    public static func instance(_ service:TestableAPi.Type) -> URLSession{
        if App.Testing.shouldUseLocalTestData {
            let mocks  = service.mocks()
            var allMocks = [URL?: (error: Error?, data: Data?, response: HTTPURLResponse?)]()
            for item in mocks{
                
            if let file  = App.Testing.responsePath(item: item.file ){
                let data = URLSession.shared.readLocalJsonFromBundle(forBundlePath:file.toUrl)
            let urlString = (item.url)
                allMocks[item.url.toUrl] = URLSession
                    .mockResponse(
                        urlString: urlString,
                        responseData: data)
            }
